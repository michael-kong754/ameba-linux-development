...
struct uart_t {
	...
	int  (*deinit)(int fd, struct uart_t *u, struct termios *ti);
...
static int realtek_init(int fd, struct uart_t *u, struct termios *ti)
{
	...
	RS_INFO("Realtek Bluetooth init uart with init speed:%d, type:HCI UART H4", u->init_speed);
	return rtb_init(fd, u->proto, u->speed, ti);
}

static int realtek_deinit(int fd, struct uart_t *u, struct termios *ti)
{

	RS_INFO("Realtek Bluetooth deinit");
	return rtb_deinit(fd, u->proto, u->speed, ti);
}
...
struct uart_t uart[] = {
	{ "any",     0x0000, 0x0000, HCI_UART_H4,   115200, 115200, FLOW_CTL, DISABLE_PM, NULL, NULL, NULL},

	/* Realtek Bluetooth H4 */
	{ "rtk_h4",  0x0000, 0x0000, HCI_UART_H4,  115200,  115200, 0, DISABLE_PM, NULL, realtek_init, realtek_deinit, realtek_post },
...
static int init_uart(char *dev, struct uart_t *u, int send_break, int raw, bool mp_mode)
{
	struct termios ti;
	int fd, i;
...
static void usage(void)
{
	...
	RS_INFO("Usage:");
	RS_INFO("\thciattach [-n] [-p] [-b] [-r] [-m mp_ant] [-t timeout] [-s initial_speed] <tty> <type | id> [speed] [flow|noflow] [bdaddr]");
	RS_INFO("\thciattach -l");
}
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "bnpt:s:lrm:")) != EOF) {
		switch (opt) {
		case 'b':
			send_break = 1;
			break;
	...
	if (mp_mode) {
		if ((mp_ant_switch != 0) && (mp_ant_switch != 1)) {
			RS_INFO("mp_ant_switch %d error, must be 0 or 1", mp_ant_switch);
			usage();
			exit(1);
		}
	}
	set_bluetooth_mode(mp_mode);
...
start:
	...
	n = init_uart(dev, u, send_break, raw, mp_mode);
	if (n < 0) {
	...
	if (mp_mode == false) {
		...
		if (p.revents & (POLLERR | POLLHUP)) {
			...
			if (u->deinit && u->deinit(n, u, NULL) < 0) {
				RS_ERR("Deinit fail");
			}
		}
	}
