...
int h4_vendor_read_rom_ver(int fd)
{
	...
	fd_rlv = open("/dev/misc-ioctl", O_RDWR);
	if (fd_rlv < 0) {
		printf("Fail to open misc device node.\n");
		return -1;
	}
	result = ioctl(fd_rlv, RTK_MISC_IOC_RLV, &rom_ver);
	if (result < 0) {
...
{
	int result;
	uint8_t buf[1 + 3 + HCI_WRITE_IQK_DATA_LEN];

	buf[0] = 0x01;
	buf[1] = 0x91;
    buf[2] = 0xfd;
    buf[3] = (uint8_t)(HCI_WRITE_IQK_DATA_LEN);
    hci_platform_get_iqk_data(phy_efuse, &buf[4], buf[3]);

	RS_INFO("%s: Issue hci write iqk cmd", __func__);

	result = start_xfer_wait(fd, buf, sizeof(buf), 1000, 0, NULL, 0);
	if (result < 0) {
		RS_ERR("%s: Failed to send reset cmd", __func__);
		return -1;
	}
