...
// This file is used for linux adb host to scan usbs.
                ...
                (const char *, const char *, unsigned char, unsigned char, int, int, unsigned))
{
    ...
    DBGX("[ find_usb_device base: %s ]\n", base);

    busdir = opendir(base);
    if(busdir == 0) return;

    DBGX("[ find_usb_device start ]\n");
    ...
    while((de = readdir(busdir)) != 0) {
        ...
        while((de = readdir(devdir))) {
            ...
            DBGX("[ %s is V:%04x P:%04x ]\n", devname, vid, pid); ///dev/bus/usb/001/010 is V:12d1 P:107e
                            ...
                            int interface, int serial_index, unsigned zero_mask)
{
    ...
    D("[ usb located new device %s (%d/%d/%d) ]\n",
        dev_name, ep_in, ep_out, interface);   //[ usb located new device /dev/bus/usb/001/010 (132/3/2) ]
    ...
    if(usb->desc < 0) {
        ...
        D("[ usb open %s fd = %d]\n", usb->fname, usb->desc); //[ usb open /dev/bus/usb/001/010 fd = 8]
...
void usb_init()
{
    ...
    //device_poll_thread(NULL);
    /*pthread_t id;
    int p_id;
    int pret;
    int join_ret;

    pret = pthread_create(&tid, NULL, device_poll_thread, &id);

    if (pret == 0) {
		printf("Thread %d ok\n", pret);
        join_ret = pthread_detach(tid);
        if (join_ret != 0) {
    ...
    pthread_create(&id, &thread_attr, device_poll_thread, &schedule_param);

    if(adb_thread_create(&tid, device_poll_thread, NULL)){
        fatal_errno("cannot create input thread");
    }

    D("Created usb_init done!\n");
}
