...
int service_to_fd(const char *name)
{
    ...
    if(!strncmp(name, "tcp:", 4)) {
...
#if ADB_HOST
            adb_mutex_lock(&dns_lock);
            D("host  socket_network_client: %s, %d\n", name + 1, port);
        ...
        D("socket_local_client: %s, %d\n", name + 6, ANDROID_SOCKET_NAMESPACE_RESERVED);
        ret = socket_local_client(name + 6,
                ANDROID_SOCKET_NAMESPACE_RESERVED, SOCK_STREAM);
    } else if(!strncmp(name, "localreserved:", 14)) {
        D("socket_local_client: %s, %d\n", name + 14, ANDROID_SOCKET_NAMESPACE_RESERVED);
        ret = socket_local_client(name + 14,
                ANDROID_SOCKET_NAMESPACE_RESERVED, SOCK_STREAM);
    } else if(!strncmp(name, "localabstract:", 14)) {
        D("socket_local_client: %s, %d\n", name + 14, ANDROID_SOCKET_NAMESPACE_ABSTRACT);
        ret = socket_local_client(name + 14,
                ANDROID_SOCKET_NAMESPACE_ABSTRACT, SOCK_STREAM);
    } else if(!strncmp(name, "localfilesystem:", 16)) {
        D("socket_local_client: %s, %d\n", name + 16, ANDROID_SOCKET_NAMESPACE_FILESYSTEM);
        ret = socket_local_client(name + 16,
                ANDROID_SOCKET_NAMESPACE_FILESYSTEM, SOCK_STREAM);
#endif
#if ADB_HOST
    } else if(!strncmp("dns:", name, 4)){
        D("host socket_local_client: %s\n", name + 4);
