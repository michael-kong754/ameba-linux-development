                        ...
                        unsigned progress)
{
    ...
    while(sz > 0) {
        ...
        ptr += xfer;
        if(progress) {
            D("sending: '%s' %4d%%    \r", fn, (int)(100LL - ((100LL * sz) / (total))));
            fflush(stdout);
        }
    }
    if(progress) {
...
static int backup(int argc, char** argv) {
    ...
    D("Now unlock your device and confirm the backup operation.\n");
...
static int restore(int argc, char** argv) {
    ...
    D("Now unlock your device and confirm the restore operation.\n");
