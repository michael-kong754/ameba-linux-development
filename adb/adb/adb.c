...
static void send_auth_publickey(atransport *t)
{
    ...
    //ret = adb_auth_get_userkey(p->data, sizeof(p->data)); //todo
    if (!ret) {
...
void handle_packet(apacket *p, atransport *t)
{
    ...
    case A_OPEN: /* OPEN(local-id, 0, "destination") */
        if (t->online) {
            ...
            s = create_local_service_socket(name); // Execute the corresponding background service thread according to the command
                                                                        // The communication between local socket and background service thread is realized through socket pair or Pty / PTS
            if(s == 0) {
                send_close(0, p->msg.arg0, t);
            } else {
                D("p->msg.arg0: %d\n", p->msg.arg0);
                s->peer = create_remote_socket(p->msg.arg0, t); // remote socket is used to communicate between adb host and adbd
                s->peer->peer = s;
...
int adb_main(int is_daemon, int server_port)
{
    ...
    D("init_transport_registration\n");
    init_transport_registration();
    D("init_transport_registration after\n");
	...
	*/
        ...
        char local_name[30];
        //build_local_name(local_name, sizeof(local_name), server_port); //todo..this is to build a smartsocket, local create a socket server, listen port is 5037
        D("install_listener igone\n");
        //if(install_listener(local_name, "*smartsocket*", NULL, 0)) {
        //    exit(1);
        //}
    }
